...
def client_info(df):
    ...
    'Кредиты берут люди с доходом около 1 МРОТ до 2 МРОТ'
...
def target_corr(df):
    ...
    'В среднем 13% людей откликнулось на маркетинговые предложения'
    ...
    f'Большинство клиентов не на пенсии и работают, работающих пенсионеров всего {df[(df.status_pens == "пенсионер") & (df.status_work == "работает")].shape[0] / df[(df.status_pens == "пенсионер")].shape[0]:0.1%}' \
    f' - их {df[(df.status_pens == "пенсионер") & (df.status_work == "работает")].shape[0]} человек'

    'Итак, почти все пенсионеры берущие кредиты - трудоустроены, так как иначе они не смогут выплатить кредит'
    ...
    '**Вывод:** На маркетинговую кампанию значительно больше реагировали не пенсионеры и работающие клиенты, значительного отличия реакции на маркетинговую кампанию по признаку наличия квартиры и машины не наблюдается'
...
def num_features(df):
    ...
    '*Целевая переменная почти не коррелирует ни с одним числовым признаком*'
...
if __name__ == '__main__':
    ...
    '**Цель проекта:** исследовать основные зависимости в данных клиентов банка'

    '**Основные термины в проекте:**'
    with st.expander("👁️‍🗨️ Признаки клиентов"):
        """
        - AGE	— возраст клиента;
        - GENDER — пол клиента;
    ...
    'Проценты на графиках - процент откликнувшихся на маркетинговую кампанию среди всех клиентов из группы'
    'МРОТ - минимальный размер оплаты труда'

    tab1, tab2, tab3 = st.tabs(
        ["Общее инфо о клиентах", "Связь с целевой переменной", 'Корреляция вещественных признаков'])
    with tab1:
        client_info(df)
    with tab2:
        target_corr(df)
